...
export default function App() {
  ...
  // [핵심] 자막 요청 (Netlify Functions 호출)
  const getTranscript = async (title, videoId) => {
    setTranscriptModal({ isOpen: true, videoId, title, content: '', loading: true, status: 'Netlify 서버 요청 중...' });
    try {
      // Netlify Function 경로 호출
      ...
      setTranscriptModal(p => ({...p, loading: false, content: '자막을 불러오지 못했습니다. (Netlify 서버 오류 또는 자막 없음)', status: '실패'}));
    }
  };
